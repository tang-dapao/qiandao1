...
### 坑 12：`_looks_like_robot_list()` 被 dump 残留节点误伤 → 导航空转（2026-09-10 发现，未修）
...
- **本质**：MuMu/WebView 的 dump 穿透已知问题（代码注释里多处提到），但用"**任意**节点文本
  存在"做否决判据会被残留节点污染。
- **候选修法**：否决不决"是否有 发消息/任务中心 节点"，改为"**当前前台 Activity/焦点**
  是否仍在 QQ 且非聊天页"；或对 dump 做新鲜度/层级校验后再用。
...
- test_flow_ocr_close.py **63**（F8 关闭语义 + F11 **banner 盲点禁令** + **覆盖层闸门** +
  跨词元拼接 + BACK 封顶 + 条带高度断言）
  + test_adb_cache.py **18**（+4 超时看门狗）
  + test_run_all_flags.py **29**（单会话连看 + 首轮广告 + 配额收尾/算术退出 + 白名单）
  + test_nav_optimize.py **37**（+3 滚动收集不被截断）
  + test_task_safety.py **32**（签到/反馈断言 + 行内 X/Y 计数 + scroll-to-top 省 dump）
